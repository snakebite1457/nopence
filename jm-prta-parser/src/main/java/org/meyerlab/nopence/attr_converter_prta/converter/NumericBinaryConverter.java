...
public class NumericBinaryConverter extends Converter {
    ...
    private void createBinaryAttr(List<String> binValues, int i) throws Exception {
        ...
        String lBorder, uBorder;
        if (borders[0] != Double.NEGATIVE_INFINITY) {
            double lowerBorder =  Math.round(borders[0]);
            lBorder = String.valueOf(lowerBorder);
        } else {
            lBorder = "-inf";
        }

        if (borders[1] != Double.POSITIVE_INFINITY) {
            double upperBorder = Math.round(borders[1]);
            uBorder = String.valueOf(upperBorder);
        } else {
            uBorder = "inf";
        }

        String newAttrName = newAttrNameBuilder.append(" ").append(lBorder)
                .append(" - ").append(uBorder).toString();
