 ...
 * @author Dennis Meyer
 */
public class WeightedOrdinalHammingDistance implements IDistanceMeasure {


    private DimensionInformation _dimensionInformation;
    private Set<Integer> _checkedDimensions;

    public WeightedOrdinalHammingDistance(DimensionInformation dimensionInformation) {
    ...
    public IDistanceMeasure copy() {
        return new WeightedOrdinalHammingDistance(_dimensionInformation);
    }
    ...
    private double buildDistance(Point first, Point second, int curDimension) {
        ...
        if (_dimensionInformation.DimensionTypeMapping.get(curDimension) ==
            ...
            if (ordinalDimsFirst != ordinalDimsSecond) {
                // Example: 5 ordinal values, event 1 have 2 and event 2 have 3.
                // Then calc 3 - 2 / 5 = 1/5; Means the distance between
                // these two events for this attribute is 1/5;
                // Then multiply this distance with given weight

                double dimDistance = (double) Math.abs(ordinalDimsFirst -
                        ordinalDimsSecond) / (double) curOrdinalDims.size();

                dimDistance *= _dimensionInformation.DimensionWeight.get
                        (curDimension);
            ...
            distance += _dimensionInformation.DimensionWeight.get(curDimension);
        }
